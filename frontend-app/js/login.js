...
document.getElementById('register-form').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            regAlert.innerHTML = '<i class="fa-solid fa-circle-check"></i> Berhasil daftar! Silakan login.';
            regAlert.style.display = 'flex';
            setTimeout(() => switchView('login'), 2000);
        } else {
            regAlert.className = 'status-alert status-error';
            regAlert.innerHTML = '<i class="fa-solid fa-circle-xmark"></i> Gagal daftar. Nama mungkin sudah ada.';
            regAlert.style.display = 'flex';
        }
    } catch (err) {
        regAlert.className = 'status-alert status-error';
        regAlert.innerHTML = '<i class="fa-solid fa-plug-circle-xmark"></i> Server tidak merespon.';
        regAlert.style.display = 'flex';
    }
});
...
document.getElementById('login-form').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            loginAlert.innerHTML = '<i class="fa-solid fa-circle-exclamation"></i> Username/Password salah.';
            loginAlert.style.display = 'flex';
        }
    } catch (err) {
        loginAlert.className = 'status-alert status-error';
        loginAlert.innerHTML = '<i class="fa-solid fa-plug-circle-xmark"></i> Server mati.';
        loginAlert.style.display = 'flex';
    }
});